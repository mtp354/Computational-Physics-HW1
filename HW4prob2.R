...
f <- function(r, t){
  x_dot <- r[3] 
  y_dot <- r[4]
  vx_dot <- ((-G*M)/(4*(sqrt((r[1])^2 + (r[2])^2)^3)))*r[1]
  vy_dot <- ((-G*M)/(4*(sqrt((r[1])^2 + (r[2])^2)^3)))*r[2]
...
t_points <- t_points[-length(t_points)]
df <- data.frame(x_points, y_points)
df %>% ggplot(aes(x = x_points, y = y_points)) + geom_point()
r_log <- log(sqrt(x_points^2 + y_points^2))
df %>% ggplot(aes(x = t_points, y = r_log)) + geom_point()
